 ...
 * @author plamere
 *
 * Modified by Sten Anderson to accommodate the Music Explorer.
 */
public class FlickrManager {
    ...
    private static String FLICKR_ID = KeyHelper.getAPIKey("flickr");
    private static String FLICKR_SECRET = KeyHelper.getAPIKey("flickr_secret");
    ...
    public java.awt.Image getImage(String name) {
        Image[] images = getPhotosForArtist(name, 1);

        if (images.length > 0) {
            ImageIcon icon = null;
            try {
                icon = new ImageIcon(new URL(images[0].getSmallImageUrl()));
        ...
        return null;
        //System.out.println ("FlickrManager found " + list.size() + " image(s) for " + name);

    }

    public List<String> getImageUrls(String name) {
        Image[] images = getPhotosForArtist(name, 20);
...
*/
    ...
    private List<Image> getImagesByTags(String[] tags, boolean all, int max, boolean interesting) {
        ...
        flickr.setSharedSecret(FLICKR_SECRET);
